...
for (let i in process.argv) {
  ...
  if (arg === '--') {
    innerArgs = process.argv.splice(i).splice(1);
    debug('Found inner arguments, probably for qbs command:', innerArgs);
    break;
  }
}
...
const commands = {
  async qbs(argv) {
    ...
    switch (innerArgs[0]) {
      case 'help': break;
      default: args = args.concat(argsFromOptions(argv)); break;
    }
